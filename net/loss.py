...
class aed_pos(nn.Module):
    ...
    def forward(self, aed_pred, aed_label):
        ...
        diversity = torch.div(aed_pred, density + 1e-10) * aed_label[:, 1:2, :, :]
        # aver_diversity = diversity.sum(dim=1, keepdim=True) * 1 / 4 * aed_label[:, 1:2, :, :]
        aver_diversity = (diversity[:, 0:1, :, :] + diversity[:, 1:2, :, :] + diversity[:, 2:3, :, :] + diversity[:, 3:4, :, :]) / 4 * aed_label[:, 1:2, :, :]
        # print(diversity[aver_diversity != 0].shape)
        # print(torch.where(aver_diversity != 0))
        nums = aed_label[:, 1, :, :].sum(dim=1, keepdim=True).float()
        tem_aver = torch.cat((aver_diversity, aver_diversity, aver_diversity, aver_diversity), dim=1)
        pull = 0.0
        push = 0.0
        pull = (diversity - tem_aver).pow(2).sum(dim=1, keepdim=True) * aed_label[:, 1:2, :, :]
        # pull = pull / (nums + 1e-4)
        ...
        for batch in zip(aver_diversity):
            ...
            tem_mask = tem_mask.unsqueeze(1) + tem_mask.unsqueeze(2)
            tem_mask = tem_mask.eq(2)
            num1 = nums.unsqueeze(2)
            dist = aed_data.unsqueeze(1) - aed_data.unsqueeze(2)
            dist = 1 - torch.abs(dist)
            dist = nn.functional.relu(dist, inplace=True)
            # print(dist.shape, num1.shape)
            dist = dist / (num * (num - 1) + 1e-4)
            # dist = dist / (num + 1e-4)
            # print(dist.shape, tem_mask.shape)
            dist = dist[tem_mask]
            push += dist.sum()
            ind += 1
        # push = push / max(1.0, num * (num - 1))
        # push = push / max(1.0, num)
        '''
